...
def test_missing_user_paths(client):
    ...
    assert client.get('/api/v1/notifications/unread/count', headers=headers).status_code == 404
...
def test_security_headers_production(client, admin_token, monkeypatch):
    ...
    assert 'upgrade-insecure-requests' in response.headers.get('Content-Security-Policy', '')
