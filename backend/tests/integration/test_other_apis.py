...
def client(tmp_path):
    ...
    approvals = [
        ...
        {"id": 2, "title": "SOP変更承認", "status": "pending", "type": "sop_change"},
        {
            "id": 3,
            "title": "完了済み承認",
            "status": "approved",
...
class TestErrorHandlers:
    ...
    def test_404_error_handler(self, client):
        ...
        response = client.get(
            "/api/v1/nonexistent_endpoint", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_invalid_token_error(self, client):
        """無効なトークンエラー"""
        response = client.get(
            "/api/v1/knowledge", headers={"Authorization": "Bearer invalid_token_here"}
        )
