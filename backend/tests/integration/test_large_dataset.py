...
class TestLargeDatasetPerformance:
    ...
    def test_search_performance_with_1000_records(self, client, auth_headers, large_knowledge_dataset):
        """
        ...
        response = client.get(
            '/api/v1/knowledge/search?query=データ',
        ...
        assert elapsed_time < 1.0, f"検索に{elapsed_time:.2f}秒かかりました（目標: 1秒以内）"
    ...
    def test_pagination_with_large_dataset(self, client, auth_headers, large_knowledge_dataset):
        """
        ...
        response1 = client.get(
            '/api/v1/knowledge?page=1&per_page=50',
            headers=auth_headers
        )
        ...
        response2 = client.get(
            '/api/v1/knowledge?page=2&per_page=50',
            headers=auth_headers
        )
        ...
        if 'data' in data1 and 'data' in data2:
            page1_ids = [item['id'] for item in data1['data'] if isinstance(item, dict) and 'id' in item]
            page2_ids = [item['id'] for item in data2['data'] if isinstance(item, dict) and 'id' in item]

            # IDが重複していないこと
            overlap = set(page1_ids) & set(page2_ids)
            assert len(overlap) == 0, f"ページ1とページ2でデータが重複しています: {overlap}"

    def test_category_filter_with_large_dataset(self, client, auth_headers, large_knowledge_dataset):
        """
    ...
    def test_sorting_with_large_dataset(self, client, auth_headers, large_knowledge_dataset):
        """
        大規模データセットでのソート機能テスト
...
class TestRateLimitingAdvanced:
    ...
    def test_login_rate_limit_5_per_15_minutes(self, client):
        ...
        try:
            ...
            assert any(code == 429 for code in status_codes) or \
                   all(code in [400, 401] for code in status_codes), \
...
class TestTimeoutHandling:
    ...
    def test_request_timeout_handling(self, client, auth_headers):
        ...
        response = client.get(
            '/api/v1/knowledge/search?query=test',
            headers=auth_headers
        )
...
class TestDataConsistency:
    ...
    def test_pagination_consistency(self, client, auth_headers):
        ...
        if 'data' in data1 and 'data' in data2:
            ...
            # データが完全に同一でないこと
            assert page1_data != page2_data or len(data1['data']) == 0, \
                "ページ1とページ2のデータが同一です"
    ...
    def test_filter_combination_consistency(self, client, auth_headers):
        ...
        # カテゴリ + ステータスフィルタ
        response = client.get(
            '/api/v1/knowledge?category=安全衛生&status=published',
            headers=auth_headers
        )
