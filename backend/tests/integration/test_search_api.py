...
class TestUnifiedSearch:
    ...
    def test_unified_search_returns_knowledge_results(self, client):
        ...
        assert any('施工計画' in item['title'] for item in data['data']['knowledge']['items'])
...
class TestSearchHelperFunctions:
    ...
    def test_search_in_fields_title_has_higher_score(self):
        ...
        _, score1 = app_v2.search_in_fields(item1, 'keyword', ['title', 'summary', 'content'])
        _, score2 = app_v2.search_in_fields(item2, 'keyword', ['title', 'summary', 'content'])
        _, score3 = app_v2.search_in_fields(item3, 'keyword', ['title', 'summary', 'content'])
...
class TestSearchAccessLogging:
    ...
    def test_unified_search_is_logged(self, client, tmp_path):
        ...
        # resource_idは数字でない文字列の場合detailsに格納される
...
class TestKnowledgeSearchIntegration:
    ...
    def test_knowledge_search_basic(self, client):
        ...
        assert data['success'] is True
        # 品質管理マニュアルがヒット
        assert any('品質' in item['title'] or '品質' in item.get('summary', '')
