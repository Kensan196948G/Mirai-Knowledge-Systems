...
class TestMetricsCompleteCoverage:
    ...
    def test_metrics_summary_system_stats(self, client, auth_headers):
        ...
        assert (
            "cpu_percent" in system or "cpu_usage_percent" in system or len(system) >= 0
        )
...
class TestSOPRecommendations:
    ...
    def test_get_related_items_with_algorithm_hybrid(self, client, auth_headers):
        ...
        response = client.get(
            "/api/v1/sop/1/related?algorithm=hybrid", headers=auth_headers
        )
        assert response.status_code in (200, 404)
    ...
    def test_get_related_items_invalid_algorithm(self, client, auth_headers):
        """無効なアルゴリズムでエラー"""
        response = client.get(
            "/api/v1/sop/1/related?algorithm=invalid", headers=auth_headers
        )
        assert response.status_code in (200, 400, 404)
    ...
    def test_get_related_items_with_min_score(self, client, auth_headers):
        ...
        response = client.get(
            "/api/v1/sop/1/related?min_score=0.5", headers=auth_headers
        )
        assert response.status_code in (200, 404)
...
class TestAuditLogFilteringExtended:
    ...
    def test_audit_logs_start_date_filter(self, client, auth_headers):
        """開始日付フィルタ"""
        response = client.get(
            "/api/v1/logs/access?start_date=2025-01-01T00:00:00", headers=auth_headers
        )
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_audit_logs_end_date_filter(self, client, auth_headers):
        """終了日付フィルタ"""
        response = client.get(
            "/api/v1/logs/access?end_date=2099-12-31T23:59:59", headers=auth_headers
        )
        assert response.status_code == 200
    ...
    def test_audit_logs_invalid_start_date(self, client, auth_headers):
        """無効な開始日付（スキップされる）"""
        response = client.get(
            "/api/v1/logs/access?start_date=invalid-date", headers=auth_headers
        )
    ...
    def test_audit_logs_invalid_end_date(self, client, auth_headers):
        """無効な終了日付（スキップされる）"""
        response = client.get(
            "/api/v1/logs/access?end_date=invalid-date", headers=auth_headers
        )
        assert response.status_code == 200
...
class TestRecommendationCache:
    ...
    def test_clear_recommendation_cache_non_admin(
