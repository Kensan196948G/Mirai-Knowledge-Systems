...
class TestAuditLogsAPI:
    ...
    def test_get_access_logs_pagination(self, client, auth_headers):
        """監査ログのページネーションが機能する"""
        response = client.get('/api/v1/logs/access?page=1&per_page=5', headers=auth_headers)
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_get_access_logs_filter_status(self, client, auth_headers):
        """ステータスでフィルタリングできる"""
        response = client.get('/api/v1/logs/access?status=success', headers=auth_headers)
        assert response.status_code == 200
        data = response.get_json()
        assert data['filters']['status'] == 'success'

    def test_get_access_logs_non_admin_forbidden(self, client, create_test_users, viewer_token):
    ...
    def test_get_access_logs_stats_non_admin_forbidden(self, client, create_test_users, viewer_token):
