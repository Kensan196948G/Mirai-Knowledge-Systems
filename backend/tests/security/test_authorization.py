...
class TestPrivilegeEscalation:
    ...
    def test_user_cannot_modify_own_roles(self, client, viewer_token, tmp_path):
        ...
        response = client.put('/api/users/3',  # viewer_userのID
                            headers={'Authorization': f'Bearer {viewer_token}'},
                            json={
                                'roles': ['admin', 'editor', 'viewer']  # 管理者権限追加試行
                            })
    ...
    def test_token_manipulation_role_claim(self, client):
        """トークンのロールクレーム改ざん防止"""
        import jwt
        ...
        original_token = login_response.get_json()['access_token']

        # トークンをデコード
        decoded = jwt.decode(original_token, options={"verify_signature": False})

        # ロールを改ざん（管理者に変更）
        decoded['roles'] = ['admin', 'editor', 'viewer']
    ...
    def test_role_verification_on_each_request(self, client, editor_token, tmp_path):
        """各リクエストでロールが検証されること"""
        # 編集者でアクセス
        response1 = client.post('/api/knowledge',
                              headers={'Authorization': f'Bearer {editor_token}'},
                              json={
                                  'title': 'Test',
                                  'summary': 'Test',
        ...
        for user in users:
            if user['username'] == 'editor_user':
                user['roles'] = ['viewer']  # 編集権限を削除
        users_file.write_text(json.dumps(users, ensure_ascii=False, indent=2))

        # 同じトークンで再度アクセス試行
        response2 = client.post('/api/knowledge',
                              headers={'Authorization': f'Bearer {editor_token}'},
                              json={
                                  'title': 'Test 2',
                                  'summary': 'Test',
...
class TestRoleConsistency:
    ...
    def test_user_roles_are_validated_on_login(self, client, create_test_users):
        ...
        assert 'user' in data
        assert 'roles' in data['user'] or 'role' in data['user']
