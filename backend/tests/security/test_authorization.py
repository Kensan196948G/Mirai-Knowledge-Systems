...
def admin_token(client, create_test_users):
    ...
    response = client.post(
        "/api/v1/auth/login", json={"username": "admin_user", "password": "admin123"}
    )
...
def editor_token(client, create_test_users):
    """編集者トークンを取得"""
    response = client.post(
        "/api/v1/auth/login", json={"username": "editor_user", "password": "editor123"}
    )
    return response.get_json()["data"]["access_token"]
...
def viewer_token(client, create_test_users):
    """閲覧者トークンを取得"""
    response = client.post(
        "/api/v1/auth/login", json={"username": "viewer_user", "password": "viewer123"}
    )
    return response.get_json()["data"]["access_token"]
...
def no_role_token(client, create_test_users):
    """ロールなしユーザーのトークンを取得"""
    response = client.post(
        "/api/v1/auth/login", json={"username": "no_role_user", "password": "norole123"}
    )
    return response.get_json()["data"]["access_token"]
...
class TestRoleBasedAccessControl:
    ...
    def test_viewer_can_read_knowledge(self, client, viewer_token):
        """閲覧者がナレッジを閲覧できること"""
        response = client.get(
            "/api/v1/knowledge", headers={"Authorization": f"Bearer {viewer_token}"}
        )
...
class TestUnauthorizedAccess:
    ...
    def test_no_role_user_denied_all_operations(self, client, no_role_token):
        ...
        # ナレッジ閲覧
        response = client.get(
            "/api/v1/knowledge", headers={"Authorization": f"Bearer {no_role_token}"}
        )
        assert response.status_code in [403, 401]
...
class TestPrivilegeEscalation:
    ...
    def test_token_manipulation_role_claim(self, client, viewer_token):
        ...
        # 改ざんされたトークンでアクセス試行
        response = client.get(
            "/api/v1/knowledge", headers={"Authorization": f"Bearer {tampered_token}"}
        )
...
class TestHorizontalAccessControl:
    ...
    def test_user_can_access_own_data(self, client, editor_token):
        ...
        # 自分のプロフィール取得
        response = client.get(
            "/api/v1/auth/me", headers={"Authorization": f"Bearer {editor_token}"}
        )
...
class TestResourceOwnership:
    ...
    def test_user_cannot_delete_others_knowledge(self, client, editor_token, tmp_path):
        ...
        # 編集者が管理者のナレッジを削除試行
        response = client.delete(
            "/api/v1/knowledge/1", headers={"Authorization": f"Bearer {editor_token}"}
        )
...
class TestAPIEndpointProtection:
    ...
    def test_admin_endpoints_require_admin_role(
        self, client, viewer_token, editor_token
    ):
        ...
        for endpoint in admin_endpoints:
            # 閲覧者でアクセス
            response = client.get(
                endpoint, headers={"Authorization": f"Bearer {viewer_token}"}
            )
            assert response.status_code in [403, 401, 404]

            # 編集者でアクセス
            response = client.get(
                endpoint, headers={"Authorization": f"Bearer {editor_token}"}
            )
            assert response.status_code in [403, 401, 404]
...
class TestRoleConsistency:
    ...
    def test_user_roles_are_validated_on_login(self, client, create_test_users):
        ...
        response = client.post(
            "/api/v1/auth/login",
            json={"username": "editor_user", "password": "editor123"},
        )
