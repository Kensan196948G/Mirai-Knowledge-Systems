...
class TestJWTTokenValidation:
    ...
    def test_invalid_token_format_denies_access(self, client):
        ...
        for invalid_token in invalid_tokens:
            ...
            assert response.status_code == 422  # Unprocessable Entity (JWT decode error)
    ...
    def test_malformed_authorization_header_denies_access(self, client):
        ...
        for header in malformed_headers:
            response = client.get('/api/knowledge', headers=header)
            assert response.status_code == 401
    ...
    def test_token_with_invalid_signature_denies_access(self, client):
        ...
        valid_token = login_response.get_json()['access_token']
        ...
        if len(parts) == 3:
            ...
            assert response.status_code == 422  # Signature verification failed
...
class TestLogoutTokenInvalidation:
    ...
    def test_logout_endpoint_exists(self, client):
        ...
        # エンドポイントが存在し、少なくとも404でないこと
        assert response.status_code in [200, 201, 204, 501]  # 実装状況に応じて
...
class TestSecureTokenStorage:
    ...
    def test_token_not_stored_in_url(self, client):
        ...
        response_url = client.get(f'/api/knowledge?token={token}')
