...
class LoginSearchBrowseUser(HttpUser):
    ...
    def browse_knowledge_list(self):
        """ナレッジ一覧閲覧（目標: 3秒以内）"""
        with self.client.get(
            "/api/v1/knowledge", headers=self.headers, catch_response=True
        ) as response:
            if response.elapsed.total_seconds() > 3.0:
                response.failure(
                    f"一覧表示が遅い: {response.elapsed.total_seconds()}秒"
                )
    ...
    def browse_knowledge_detail(self):
        ...
        with self.client.get(
            ...
            if response.status_code == 200:
                if response.elapsed.total_seconds() > 3.0:
                    response.failure(
                        f"詳細表示が遅い: {response.elapsed.total_seconds()}秒"
                    )
...
class KnowledgeCreationUser(HttpUser):
    ...
    def create_knowledge(self):
        ...
        with self.client.post(
            "/api/v1/knowledge", headers=self.headers, json=data, catch_response=True
        ) as response:
            if response.elapsed.total_seconds() > 3.0:
    ...
    def update_knowledge(self):
        ...
        self.client.put(
            f"/api/v1/knowledge/{knowledge_id}", headers=self.headers, json=data
        )
    ...
    def delete_knowledge(self):
        """ナレッジ削除"""
        knowledge_id = random.randint(1, 50)
        self.client.delete(f"/api/v1/knowledge/{knowledge_id}", headers=self.headers)
...
class ApprovalOperationUser(HttpUser):
    ...
    def get_approval_list(self):
        """承認待ち一覧取得"""
        self.client.get("/api/v1/approvals?status=pending", headers=self.headers)
    ...
    def approve_knowledge(self):
        ...
        with self.client.post(
            ...
            if response.status_code == 200:
                if response.elapsed.total_seconds() > 3.0:
                    response.failure(
                        f"承認処理が遅い: {response.elapsed.total_seconds()}秒"
                    )
...
class NotificationCheckUser(HttpUser):
    ...
    def get_notifications(self):
        """通知一覧取得（目標: 2秒以内）"""
        with self.client.get(
            "/api/v1/notifications?limit=20", headers=self.headers, catch_response=True
        ) as response:
            if response.elapsed.total_seconds() > 2.0:
                response.failure(
                    f"通知取得が遅い: {response.elapsed.total_seconds()}秒"
                )
    ...
    def mark_notification_read(self):
        ...
        self.client.post(
            f"/api/v1/notifications/{notification_id}/read", headers=self.headers
        )
...
class DashboardRefreshUser(HttpUser):
    ...
    def get_dashboard_stats(self):
        """ダッシュボード統計（目標: 3秒以内）"""
        with self.client.get(
            "/api/v1/dashboard/stats", headers=self.headers, catch_response=True
        ) as response:
            if response.elapsed.total_seconds() > 3.0:
                response.failure(
                    f"統計取得が遅い: {response.elapsed.total_seconds()}秒"
                )

    @task(3)
    def get_recent_activities(self):
        """最近のアクティビティ取得"""
        self.client.get("/api/v1/dashboard/activities?limit=10", headers=self.headers)

    @task(2)
    def get_popular_knowledge(self):
        """人気ナレッジ取得"""
        self.client.get("/api/v1/dashboard/popular?limit=5", headers=self.headers)
    ...
    def unified_search(self):
        ...
        with self.client.get(
            ...
            if response.elapsed.total_seconds() > 2.0:
                response.failure(
                    f"横断検索が遅い: {response.elapsed.total_seconds()}秒"
                )
