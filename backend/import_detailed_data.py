...
from models import Knowledge, SOP, Incident, Consultation
        ...
        for item in data:
            ...
            if 'steps' in item:
                ...
                for step in item['steps']:
                    content_parts.append(f"ステップ{step['step_number']}: {step['description']}\n")
            ...
            sop = SOP(
                ...
                revision_date=parse_datetime(item.get('revision_date')) or datetime.utcnow().date(),
...
def import_incident_details():
    ...
    try:
        for item in data:
            ...
            if existing:
                # 既存データを更新
                if item.get('description') and len(item['description']) > len(existing.description or ''):
            ...
            # 新規作成
            incident_date = parse_datetime(item.get('occurred_at') or item.get('incident_date'))
            if incident_date:
