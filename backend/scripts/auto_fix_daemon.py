...
class AutoFixDaemon:
    ...
    def _restart_service(self, service_name: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.error(f"サービス再起動失敗: {service_name} - {result.stderr}")
                return False
        ...
        try:
            ...
            for log_file in Path(log_dir).glob('*.log'):
                if log_file.stat().st_size > 10 * 1024 * 1024:  # 10MB以上
                    ...
                    self.logger.info(f"ログローテーション完了: {log_file.name} -> {archive_name}")
    ...
    def _fix_permissions(self, paths: List[str], owner: str = None, mode: str = None) -> bool:
        """ファイル権限を修正"""
        try:
            base_dir = os.path.dirname(os.path.dirname(__file__))
            ...
            for path in paths:
                ...
                if owner:
                    subprocess.run(['sudo', 'chown', '-R', owner, full_path], check=True)
        ...
        try:
            ...
            for json_file in Path(data_dir).glob('*.json'):
                try:
                    ...
                    self.logger.warning(f"破損したJSONファイルを修復: {json_file.name} (バックアップ: {backup_file.name})")
    ...
    def _analyze_logs(self) -> bool:
        ...
        try:
            ...
            if error_stats:
                self.logger.info(f"エラー統計: {json.dumps(error_stats, ensure_ascii=False)}")
    ...
    def _create_backup(self):
        ...
        try:
            ...
            if os.path.exists(data_dir):
                ...
                subprocess.run(
                    ['tar', '-czf', backup_path, '-C', base_dir, 'data'],
                    check=True
                )
