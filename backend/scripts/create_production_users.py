...
def main():
    ...
    for u in users:
        print(
            f"  - {u['username']} ({u.get('full_name', 'N/A')}) - ロール: {u.get('roles', [])}"
        )
    print()
    ...
    engineer_password = (
        os.environ.get("ENGINEER_PASSWORD") or generate_secure_password()
    )
    ...
    for user_data in production_users:
        ...
        if result:
            ...
            print(
                f"[OK] ユーザー作成: {user_data['username']} ({user_data['full_name']})"
            )
