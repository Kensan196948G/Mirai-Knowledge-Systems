...
from models import Knowledge, SOP, Regulation, Incident, User
...
class DataValidator:
    ...
    def validate_incident(data: Dict) -> tuple[bool, Optional[str]]:
        """事故レポートデータをバリデーション"""
        required_fields = ['title', 'description', 'project', 'incident_date', 'severity']
...
class DataMigrationTool:
        ...
        try:
            ...
            for s in sop_list:
                backup_data['sop'].append({
                    ...
                    'category': s.category,
                    'version': s.version,
                    'revision_date': s.revision_date.isoformat() if s.revision_date else None,
                    'target': s.target,
                    'tags': s.tags,
                    'content': s.content,
                    'status': s.status
    ...
    def restore_backup(self, backup_id: str):
        ...
        if not backup_path.exists():
            raise DataMigrationError(f"バックアップファイルが見つかりません: {backup_id}")
    ...
    def import_knowledge_from_csv(
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row_num, row in enumerate(reader, start=2):  # ヘッダーを考慮して2から
                    try:
                        # データ準備
                        data = {
                        ...
                        # 重複チェック
                        existing = self.db.query(Knowledge).filter_by(
                            title=data['title'],
                            category=data['category']
                        ).first()

                        if existing:
                            logger.warning(f"行 {row_num}: 重複データをスキップ - {data['title']}")
                            result.add_skip()
                            continue
                        ...
                        if preview:
                            ...
                            logger.info(f"インポート成功 (ID: {knowledge.id}): {data['title']}")
                            result.add_success()
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for row_num, row in enumerate(reader, start=2):
                    try:
                        ...
                        revision_date = datetime.strptime(revision_date_str, '%Y-%m-%d').date() if revision_date_str else date.today()
                        ...
                        # 重複チェック
                        existing = self.db.query(SOP).filter_by(
                            title=data['title'],
                            version=data['version']
                        ).first()

                        if existing:
                            logger.warning(f"行 {row_num}: 重複データをスキップ - {data['title']} v{data['version']}")
                            result.add_skip()
                            continue

                        if preview:
                            logger.info(f"[プレビュー] 行 {row_num}: {data['title']} v{data['version']}")
                            result.add_success()
                        else:
                            sop = SOP(**data)
                            self.db.add(sop)
                            ...
                            logger.info(f"インポート成功 (ID: {sop.id}): {data['title']}")
                            result.add_success()
    ...
    def import_from_json(
        ...
        try:
            ...
            for idx, s_data in enumerate(data.get('sop', []), start=1):
                try:
                    # 日付を変換
                    if 'revision_date' in s_data and isinstance(s_data['revision_date'], str):
                        s_data['revision_date'] = datetime.strptime(s_data['revision_date'], '%Y-%m-%d').date()
...
def main():
    ...
    parser_rollback.add_argument(
        '--backup-id',
        required=True,
        help='バックアップID (例: 20250127_123456)'
    )
