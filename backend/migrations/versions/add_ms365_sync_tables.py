...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "folder_path", sa.String(length=500), server_default="/", nullable=True
        ),
        ...
        sa.Column(
            "metadata_mapping", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
    ...
    # インデックス作成
    op.create_index(
        "idx_ms365_sync_config_enabled",
        "ms365_sync_config",
        ["is_enabled"],
        schema="public",
    )
    op.create_index(
        "idx_ms365_sync_config_next_sync",
        "ms365_sync_config",
        ["next_sync_at"],
        schema="public",
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["config_id"], ["public.ms365_sync_config.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(
    ...
    # インデックス作成
    op.create_index(
        "idx_ms365_sync_history_config",
        "ms365_sync_history",
        ["config_id"],
        schema="public",
    )
    op.create_index(
        "idx_ms365_sync_history_status",
        "ms365_sync_history",
        ["status"],
        schema="public",
    )
    op.create_index(
        "idx_ms365_sync_history_started",
        "ms365_sync_history",
        ["sync_started_at"],
        schema="public",
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "sync_status", sa.String(length=50), server_default="pending", nullable=True
        ),
        ...
        sa.ForeignKeyConstraint(
            ["knowledge_id"], ["public.knowledge.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    # インデックス作成
    op.create_index(
        "idx_ms365_file_mapping_config",
        "ms365_file_mapping",
        ["config_id"],
        schema="public",
    )
    op.create_index(
        "idx_ms365_file_mapping_sp_file_id",
        "ms365_file_mapping",
        ["sharepoint_file_id"],
        schema="public",
    )
    op.create_index(
        "idx_ms365_file_mapping_knowledge",
        "ms365_file_mapping",
        ["knowledge_id"],
        schema="public",
    )
    op.create_index(
        "idx_ms365_file_mapping_status",
        "ms365_file_mapping",
        ["sync_status"],
        schema="public",
    )
