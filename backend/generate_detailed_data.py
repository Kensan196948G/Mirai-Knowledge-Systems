...
def generate_knowledge_details(count=100):
    ...
    for i in range(1, count + 1):
        ...
        # 関連ナレッジID
        related_ids = random.sample(
            range(1, count + 1), min(random.randint(5, 10), count - 1)
        )
...
def generate_sop_details(count=50):
    ...
    for i in range(1, count + 1):
        ...
        for idx, item in enumerate(
            random.sample(checklist_items, random.randint(5, 8)), 1
        ):
            checklist.append(
                {
                    "id": idx,
        ...
        # 関連SOP
        related_sops = random.sample(
            range(1, count + 1), min(random.randint(3, 6), count - 1)
        )
...
def generate_incidents_details(count=30):
    ...
    for i in range(1, count + 1):
        ...
        for idx, action in enumerate(
            random.sample(action_templates, random.randint(3, 5)), 1
        ):
            corrective_actions.append(
                {
                    ...
                    "deadline": (
                        incident_date + timedelta(days=random.randint(7, 30))
                    ).strftime("%Y-%m-%d"),
            ...
            answers.append(
                {
                    ...
                    "attachments": (
                        [
                            {
                                "name": f"参考資料_{a+1}.pdf",
                                "url": f"https://example.com/file{a+1}.pdf",
                            }
                        ]
    ...
    for i in range(1, count + 1):
        ...
        for m in range(random.randint(5, 10)):
            milestone_date = start_date + timedelta(
                days=int(duration_days * (m + 1) / 10)
            )
            milestones.append(
                {
                    "id": m + 1,
                    "title": (
                        milestone_templates[m]
                        if m < len(milestone_templates)
                        else f"マイルストーン{m+1}"
                    ),
...
def main():
    ...
    with open(
        os.path.join(data_dir, "knowledge_details.json"), "w", encoding="utf-8"
    ) as f:
    ...
    with open(
        os.path.join(data_dir, "incidents_details.json"), "w", encoding="utf-8"
    ) as f:
    ...
    with open(
        os.path.join(data_dir, "consultations_details.json"), "w", encoding="utf-8"
    ) as f:
