...
class Config:
    ...
    # データベース設定
    USE_POSTGRESQL = os.environ.get("MKS_USE_POSTGRESQL", "false").lower() in (
        "true",
        "1",
        "yes",
    )
    ...
    JWT_SECRET_KEY = os.environ.get(
        "MKS_JWT_SECRET_KEY", "mirai-knowledge-system-jwt-secret-key-2025"
    )
    ...
    SESSION_COOKIE_SECURE = os.environ.get("MKS_FORCE_HTTPS", "false").lower() in (
        "true",
        "1",
        "yes",
    )
    ...
    # HTTPS設定
    FORCE_HTTPS = os.environ.get("MKS_FORCE_HTTPS", "false").lower() in (
        "true",
        "1",
        "yes",
    )
    ...
    SMTP_USE_TLS = os.environ.get("MKS_SMTP_USE_TLS", "true").lower() in (
        "true",
        "1",
        "yes",
    )
    SMTP_USE_SSL = os.environ.get("MKS_SMTP_USE_SSL", "false").lower() in (
        "true",
        "1",
        "yes",
    )
    NOTIFICATION_RETRY_COUNT = int(os.environ.get("MKS_NOTIFICATION_RETRY_COUNT", "5"))
    DISABLE_EXTERNAL_NOTIFICATIONS = os.environ.get(
        "MKS_DISABLE_EXTERNAL_NOTIFICATIONS", "false"
    ).lower() in ("true", "1", "yes")
    EXTERNAL_NOTIFICATION_TYPES = (
        os.environ.get("MKS_EXTERNAL_NOTIFICATION_TYPES", "").split(",")
        if os.environ.get("MKS_EXTERNAL_NOTIFICATION_TYPES")
        else []
    )
        ...
        if has_cert:
            ...
            if not os.path.exists(cert_path):
                ...
                if not os.access(cert_path, os.R_OK):
                    errors.append(
                        f"証明書ファイルの読み取り権限がありません: {cert_path}"
                    )

        # SharePoint/OneDrive設定の検証
        if os.environ.get("MS365_SYNC_ENABLED", "false").lower() in (
            "true",
            "1",
            "yes",
        ):
            ...
            if not site_url:
                errors.append(
                    "MS365_SYNC_ENABLED=true の場合、MS365_SITE_URL が必要です"
                )
